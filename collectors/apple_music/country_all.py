...
Fetches songs + albums for each storefront in a SINGLE request
(types=songs,albums) instead of separate passes, then writes the same two CSVs
as the legacy per-type collectors (country_charts.py, country_albums.py).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect Apple Music country songs/albums charts for Taylor Swift in one pass."
    )
...
def fetch_country(session, manager: TokenManager, country: str) -> tuple[list[dict], list[dict]]:
