...
from core.config import ARTIST_ID, CHART_LIMIT, DB_DIR, WORKERS
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Collect and print the ranking, write nothing.")
...
def _song_from_item(item: dict) -> dict | None:
...
def fetch_storefront_chart_songs(session, manager: TokenManager, storefront: str) -> list[dict]:
    """Taylor Swift songs in this storefront's Apple Music Top Songs chart
    (top CHART_LIMIT, same endpoint as country_all.py), in chart order.
    A 400 = storefront without a songs chart -> [] (legit, not a failure)."""
    url = f"https://amp-api-edge.music.apple.com/v1/catalog/{storefront}/charts?types=songs&limit={CHART_LIMIT}"
    resp = _get_with_auth(session, manager, url)
    if resp.status_code == 400:
        return []
    if resp.status_code == 401:
        raise RuntimeError(f"Unauthorized after token refresh ({storefront} chart)")
    resp.raise_for_status()
    block = ((resp.json().get("results") or {}).get("songs") or [])
    items = (block[0] or {}).get("data", []) if block else []
    songs = []
    for item in items or []:
        if not is_taylor_swift_song(item, item.get("attributes", {}) or {}):
            continue
...
def merge_chart_into_top_songs(top_songs: list[dict], chart_songs: list[dict]) -> list[dict]:
    """TS ranking for one storefront = the TS songs of the live Apple Music
    chart first (chart order), then the artist-page top-songs list minus those
...
def fetch_storefront_top_songs(session, manager: TokenManager, storefront: str) -> list[dict]:
    ...
    return merge_chart_into_top_songs(songs, fetch_storefront_chart_songs(session, manager, storefront))
...
def main() -> None:
    ...
    if args.dry_run:
        print(f"[dry-run] {len(rows)} row(s) computed, nothing written")
        return
