"""Generate the not-combined TayBoard songs chart.

The filename keeps the requested `seperate` spelling for compatibility.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate not-combined TayBoard songs chart")
    ...
    p.add_argument("--rebuild-index", dest="rebuild_index", action="store_true",
                   help="Rebuild swift_top_100_not_combined_index.json from existing snapshots and upload to R2")
...
def run_from_args(args: argparse.Namespace, *, engine: ModuleType | None = None) -> int:
    ...
    next_args.variant = "not-combined"
    engine._configure_variant("not-combined")
    try:
