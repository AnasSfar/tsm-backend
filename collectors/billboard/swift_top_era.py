"""Generate the TayBoard eras chart."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate TayBoard eras chart")
    ...
    p.add_argument("--rebuild-index", dest="rebuild_index", action="store_true",
                   help="Rebuild swift_top_eras_index.json from existing snapshot files")
...
def run_from_args(args: argparse.Namespace, *, engine: ModuleType | None = None) -> int:
    ...
    next_args.variant = "eras"
    albums._configure_variant("eras")
    try:
