...
from collectors.spotify.core.data_paths import deezer_daily_csv, deezer_daily_csv_paths
...
def _daily_csv_paths(csv_path: Path, days: int | None = None) -> list[Path]:
    paths = deezer_daily_csv_paths(csv_path.name)
    if days is None:
        return paths
    ...
    if prev_keys:
        ...
        if prev_rows == new_comparable:
            ...
            if prev_day == new_day:
                ...
                return
            # Cross-day identical: weak signal (a small TS-only track set can
            # genuinely repeat), so keep the data but leave a trace for diagnosis.
            print(
                f"[info] {csv_path.name}: snapshot identical to last known snapshot "
                f"({prev_keys[0]}) — Deezer chart may not have refreshed yet; writing anyway"
            )

    current_day = scraped_at[:10]
    csv_path = deezer_daily_csv(current_day, csv_path.name)
    filtered = [
        r for r in existing
