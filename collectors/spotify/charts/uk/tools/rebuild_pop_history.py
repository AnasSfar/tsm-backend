#!/usr/bin/env python3
"""
rebuild_pop_history.py â€” Reconstruit ts_pop_history.json depuis le dÃ©but.

Pour chaque date historique :
  1. Si ts_pop_songs.csv existe â†’ utilise pop_flag directement
  2. Sinon si ts_all_songs.csv existe â†’ filtre les lignes pop_flag=True
  3. Sinon si log.txt existe â†’ extrait les chansons TS du chart streaming,
     puis vÃ©rifie is_pop dans songs_db.json

GÃ©nÃ¨re aussi ts_pop_history.csv (date, track_name).
...
_LOG_SONG_RE = re.compile(r"^[-â€“]\s*#\d+\s*\([^)]+\)\s*(.+?)\s*\|")

# Matches pop lines in tweet.txt:  "- #37 (NEW) I Don't Wanna Live Forever"
_TWEET_POP_RE = re.compile(r"^[-â€“]\s*#(\d+)\s*\([^)]+\)\s*(.+)$")
...
def tracks_from_log(path: Path, db: dict) -> list[str]:
    ...
    # Find the streaming section (between "Spotify UK :" and "Spotify UK (Pop) :")
    m = re.search(r"Spotify UK\s*:\s*\n(.*?)(?=Spotify UK \(Pop\)|$)", text, re.DOTALL)
    if not m:
        return []
...
def tracks_from_tweet(path: Path) -> list[str]:
    """
    Parse tweet.txt and extract track names from the 'Spotify UK (Pop) :' section.
    These are already filtered to TS pop tracks so no songs_db lookup needed.
    """
    text = path.read_text(encoding="utf-8", errors="ignore")
    m = re.search(r"Spotify UK \(Pop\)\s*:\s*\n(.*?)(?:\n\n|\Z)", text, re.DOTALL)
    if not m:
        return []
    tracks = []
    for line in m.group(1).splitlines():
