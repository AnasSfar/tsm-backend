#!/usr/bin/env python3
"""Backfill Taylor Swift entries from Spotify Charts (UK daily) into db/charts_history_uk.csv.
...
Usage examples:
  python backfill_charts_history_uk.py
  python backfill_charts_history_uk.py --start 2017-01-01 --end 2026-04-03
  python backfill_charts_history_uk.py --force
"""
...
STATE_FILE = TOOLS_DIR / "json" / "backfill_uk_state.json"
DEFAULT_FAILED_DATES_CSV = TOOLS_DIR / "json" / "backfill_uk_failed_dates.csv"
CACHE_FILE_DEFAULT = TOOLS_DIR / "json" / "backfill_uk_cache.json"

DEFAULT_OUTPUT_CSV = Path(__file__).resolve().parents[6] / "db" / "charts_history_uk.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill Spotify Charts UK Taylor Swift archive CSV")
