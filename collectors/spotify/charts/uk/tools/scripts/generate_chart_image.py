#!/usr/bin/env python3
"""
generate_chart_image.py â€” gÃ©nÃ¨re le PNG du chart Taylor Swift France.
Design adaptÃ© depuis unfiltered-charts (light glassmorphism).
...
# ---------------------------------------------------------------------------
# Image URL â†’ base64 data URI (Chromium bloque les URLs externes via file://)
# ---------------------------------------------------------------------------
...
def url_to_data_uri(url: str) -> str:
    ...
    for attempt in range(2):
        try:
            ...
            last_exc = e
    print(f"[warn] url_to_data_uri: Ã©chec pour {url} ({last_exc}), URL brute utilisÃ©e (risque d'affichage vide)")
    _img_cache[url] = url
    return url
...
def pick_header_image() -> Path | None:
    """Returns a random image from the headers folder (should be 860Ã—80px)."""
...
def build_cover_map() -> dict:
    """Returns {normalized_album_title â†’ cover_url} from covers.json."""
...
def build_track_album_map() -> dict:
    """Reads album files + songs.json â†’ {normalized_track_title â†’ album_title}."""
    result = {}
...
def _get_track_image_map() -> dict:
    """Returns {normalized_track_title â†’ image_url} from album files + songs.json (lazy, cached)."""
...
def build_out_rows_html(
  ...
  <div class="col-song">
    ...
    <div class="song-info">
      ...
      <div class="song-artist">{artist} Â· last: {yesterday}</div>
    </div>
  </div>
</div>
"""
...
def build_rows_html(
    ...
    for i, row in enumerate(rows):
        ...
        # Album cover: discography lookup â†’ fallback to scraped CDN URL
        cover_url = url_to_data_uri(get_album_cover(track, track_album_map, cover_map, scraped_img))
...
def build_html(
    ...
    if pop_rows or out_songs:
        ...
        pop_section_html = f"""<div class="section-hdr" {sec_style}>UK Pop Chart</div>
{COL_HEADS_HTML}
...
<div class="container">
  <div class="hdr" {hdr_style}>
    {SPOTIFY_SVG}
    <div>
            <div class="hdr-title">Taylor Swift - UK Spotify</div>
            <div class="hdr-sub">Daily Chart - {date_fmt}</div>
    </div>
  </div>
    <div class="section-hdr" {sec_style}>UK Chart</div>
  {COL_HEADS_HTML}
  {rows_html}
  {pop_section_html}
  <div class="ftr">
...
def generate_all_headers(chart_date: str) -> list[Path]:
    """GÃ©nÃ¨re une image par photo dans headers/, nommÃ©e chart_image_<photo>.png."""
    if not HEADERS_DIR.exists():
    ...
    print(f"\n{len(results)} images gÃ©nÃ©rÃ©es dans {date_dir}/")
    return results


def generate_multi(chart_dates: list[str], header_img: Path | None = None) -> Path:
    """GÃ©nÃ¨re une seule image PNG combinant plusieurs dates (sÃ©parÃ©es par un bandeau)."""
    ...
    for chart_date in chart_dates:
        ...
        json_path = date_dir / f"ts_chart_{chart_date}.json"
        if not json_path.exists():
            print(f"  JSON introuvable pour {chart_date}, ignorÃ©")
            continue
        rows = load_json(json_path)
        if not rows:
            continue
    ...
    if not valid_dates:
        raise ValueError("Aucun JSON trouvÃ© pour les dates fournies")
    ...
    subtitle  = f"Daily Chart - {first_fmt} to {last_fmt}"
