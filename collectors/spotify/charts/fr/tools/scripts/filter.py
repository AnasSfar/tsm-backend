...
def _ensure_archive_columns() -> None:
    if not ARCHIVE_CSV.exists() or ARCHIVE_CSV.stat().st_size == 0:
        return
    import csv as _csv

    with ARCHIVE_CSV.open("r", encoding="utf-8-sig", newline="") as f:
        reader = _csv.DictReader(f)
        if reader.fieldnames and all(col in reader.fieldnames for col in ARCHIVE_COLUMNS):
            return
        rows = list(reader)

    with ARCHIVE_CSV.open("w", encoding="utf-8", newline="") as f:
        writer = _csv.DictWriter(f, fieldnames=ARCHIVE_COLUMNS)
        writer.writeheader()
        for row in rows:
