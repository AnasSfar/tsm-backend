...
def main():
    ...
    if date_args:
        ...
        if not force and already_posted(target):
            log("INFO", f"posted.lock déjà présent pour {target} — post ignoré (--force pour reposter)")
            unposted = []
        else:
            unposted = [target]
    else:
    ...
    if post_only:
        ...
        processed = [target]
        mark_updated(target)
        _date_fmt = target.strftime("%A, %B %d, %Y")
    ...
    for d in unposted:
        content, unavailable = run_filter(d, replace_date=replace_date)
        if content:
            results[d] = content
        elif unavailable:
