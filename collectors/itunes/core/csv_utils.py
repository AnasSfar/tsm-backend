"""Idempotent-by-snapshot CSV helpers for the iTunes collector.

Mirrors collectors/apple_music/core/csv_utils.py (same movement semantics:
previous_rank always means "vs the last snapshot of a previous day", never
"vs this morning's rerun"), pointed at the itunes_charts snapshot tree.
"""
...
def _daily_csv_paths(csv_path: Path, days: int | None = None) -> list[Path]:
    paths = itunes_daily_csv_paths(csv_path.name)
    if days is None:
        return paths
...
def rewrite_for_snapshot(
    ...
    """Append a new snapshot, removing any existing rows with the same scraped_at
    (idempotent). Skips the write when the new data is identical to the most
    recent existing snapshot of the same day."""
    ...
    if prev_keys:
        ...
        if prev_rows == new_comparable:
            ...
            print(
                ...
                f"({prev_keys[0]}) — iTunes chart may not have refreshed yet; writing anyway"
            )

    current_day = scraped_at[:10]
    csv_path = itunes_daily_csv(current_day, csv_path.name)
    filtered = [
        r for r in existing
...
def load_previous_ranks(
    ...
    """
    require_previous = os.getenv("ITUNES_REQUIRE_PREVIOUS_RANKS", "").strip().lower() in ("1", "true", "yes")
    rows = read_csv_rows(csv_path, include_daily_history=True, history_days=PREV_RANK_WINDOW_DAYS)
    if not rows:
        if require_previous:
            raise RuntimeError(
                f"iTunes previous-rank history missing for {csv_path.name}; "
    ...
    if not all_keys:
        if require_previous:
            raise RuntimeError(
                f"iTunes previous-day snapshot missing for {csv_path.name} before {current_day}; "
                "refusing to publish movements without a verified prior snapshot"
            )
        return {}
    latest = all_keys[0]
    ...
    for row in rows:
        ...
        try:
            ...
            continue
        key = tuple(
            (row.get(field, "") if field != song_field else rank_key(row.get(song_field, "")))
