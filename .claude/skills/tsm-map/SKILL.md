...
| `pages/AppleMusicSongDetail.jsx` | Page détail chanson Apple Music (`/amcharts/apple-music/:songId`). Sélecteur de région (`CountryPicker`, "Overall" = tout via `PlacementTables` importé de `AppleMusic.jsx`, un pays = filtré) + `CalendarPicker` pour choisir le snapshot (une seule date affichée, pas un historique complet) + histogramme des 14 derniers jours calendaires sur le Global Top 100 (jours non chartés = barre vide, ne pas sauter les jours manquants) + badge `RecordStar` best-rank-since |
