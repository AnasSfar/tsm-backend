...
- Carte mobile "Marquee" (≤700px) : visuel de `.song-row-mobile*` défini **une fois** en haut de `Streams.css` (cover 82px, rang badgé sur la pochette sans #1 spécial, daily en Bebas Neue, % pilule, total micro-label). Les blocs par surface ne font que `display` + shell. `/albums` = même look à la main sur `.album-row-mobile*` dans `Albums.css`. Le `.delta` de rang est `absolute` en coin haut-droite de la carte (`top:13px; right:13px`), `.delta.neutral` masqué ; best-day = étoile or + `.best-day-mobile-label` micro-capitales grises. Détail : `STYLE.md` §6.
