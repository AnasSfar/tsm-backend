...
- **Palettes officielles par era** : `styles/era-color-codes.json` (indépendant, non importé par le build) stocke les 12 codes bruts par era (dark1/light1/dark2/light2/accent1-6/hyperlink/followedHyperlink). Reçu une palette officielle → remplir son bloc JSON puis reporter dans `themes.css`. Fearless + Taylor Swift déjà à jour (2026-09-06) ; TS n'a pas de bloc dark dédié (palette trop pastel pour un fond sombre lisible), reste sur le scaffold dark générique.
