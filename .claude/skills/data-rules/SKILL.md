...
4. **Jamais écrire `daily=0` pour un track pending épuisé** — on n'écrit pas la ligne (affichage « -- »), un 0 inventé pollue les charts et les posts. **Exception (décision 2026-07-08) : extras à trafic quasi nul.** Un `chart_extra=true` épuisé en `same_total` reçoit `daily=0` (raison `assumed_zero_low_traffic_extra`) si sa moyenne streams/jour depuis la sortie ≤ `EXTRA_ASSUME_ZERO_MAX_AVG_DAILY` (500, dans `update_streams.py`), ou — release_date absente du catalogue — si ses dailies récents enregistrés (≥ 3 points) sont en moyenne ≤ ce seuil. Un vrai 0 est plausible pour ces remixes/lives dormants ; ils ne bloquent plus l'export/post final. Ne s'applique JAMAIS à un non-extra. Cohérence : le garde-fou `validate_released_active_history_complete` (history_store.py, appelé avant chaque artefact publiable) ne traite un « 0 à total inchangé » comme ambigu que pour les non-extra.
