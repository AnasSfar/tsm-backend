...
python update_streams.py YYYY-MM-DD          # run normal pour une date précise
  ...
  --best-day-last    best-day-since repoussé à la fin : pas de watchers early (ni calcul des candidats,
                     ni exports partiels/upload R2 associés), pas de labels best-day dans la notif grower,
                     Top Songs posté sans les replies recap, pas d'alternance en finalize ; le batch complet
                     (`_post_best_day_since` : chansons + recaps d'ère + recap global) part après les tables gainers.
                     Les marqueurs `★ · since` des cards et best_day_since.json de l'export web restent calculés.
                     Combinable avec --post-only : top20 sans replies recap, `best-day-since` forcé en dernier.
  --skip S[,S]       run normal mais les étapes listées ne postent jamais (mêmes noms que --post-only :
                     top-eras, all-albums, top20, top45, recap, weekend-gainers, milestones, overtakes,
                     best-day-since, debut, gainers). `debut` coupe aussi le poster early ; `best-day-since`
                     coupe les watchers early + replies recap du Top Songs (env TSM_SKIP_BEST_DAY_SINCE=1).
                     Filtre aussi --post-only. Run déjà lancé : seul best-day-since est encore coupable, via
...
Orchestrateur : `run_apple_music.py` (appelé par `python -m tsm collect apple-music`) — **depuis le 2026-09-25** : verrou mono-instance (`collectors/apple_music/tools/locks/run_apple_music.lock`, sortie 75 = heure sautée), alertes ntfy (`NTFY_TOPIC_APPLE_MUSIC`) sur échec collecte/export/upload/crash, posts nouvelles sorties lancés dès que `global.py` + `country_all.py` ont fini (sans attendre `genre_all.py`), et un échec de la notification Global ne bloque plus l'upload R2 ; helpers partagés `collectors/spotify/core/run_guard.py` (aussi utilisé par `run_itunes.py`). `--no-post` (skip notification), `--no-images`, `--force-images`. `scraped_at` est arrondi par défaut au dernier slot pair Europe/Paris `00/02/04/.../22` (`APPLE_MUSIC_ROUND_SCRAPED_AT`, `APPLE_MUSIC_SNAPSHOT_TZ`, `APPLE_MUSIC_SNAPSHOT_HOURS`) — **override `.env` depuis le 2026-09-24 : `APPLE_MUSIC_SNAPSHOT_HOURS=0,1,...,23`**, pour que la cadence horaire du Planificateur (voir plus bas) produise vraiment 24 snapshots/jour au lieu de re-arrondir vers le même créneau pair — et la même date est passée explicitement aux sous-collecteurs. Depuis 2026-08-30, `build_scraped_at()` écrit un timestamp **timezone-aware** (`2026-08-30T14:00:00+02:00`) au lieu d'une heure murale nue — le frontend le reconvertit dans le fuseau du visiteur (`parseCollectorTimestamp` / `formatCollectorClock` dans `tsm-frontend/frontend/src/utils/format.js`). Les vieilles lignes CSV sans offset sont traitées comme Europe/Paris côté frontend. Tous les consommateurs backend ne font que du `scraped_at[:10]` (slice date), donc le suffixe d'offset est inoffensif ; aucune migration de l'historique. Launcher : `run_apple_music.bat` (3 lignes depuis le 2026-09-24 : Apple Music → iTunes → `post_new_release_progression.py`, ce dernier no-op hors fenêtre de sortie d'un titre). Un collecteur bloquant en échec = run abandonné (pas d'export/images/upload ; `ts_page_all.py`/`finalize_ts_top_songs_daily.py`/`ts_top_songs_live.py` sont non bloquants). **Depuis le 2026-09-24 les collecteurs tournent en 4 groupes parallèles** (`SCRIPT_GROUPS` : `genre_all.py` | `ts_page_all.py` → `finalize_ts_top_songs_daily.py` → `ts_top_songs_live.py` | `country_all.py` | `global.py` → `ts_page.py`), sortie de chaque enfant imprimée en un bloc avec sa durée ; `APPLE_MUSIC_PARALLEL_SCRIPTS=0` = retour au séquentiel. Les images (cards + snapshots) sont générées **pendant** l'upload R2 (qui ne les lit pas).
...
| `ts_page_all.py` scoring note | Depuis le 2026-09-19, le composite TS Top Songs garde le score `500/rank**0.75 * poids_marche`, mais applique aussi `APPLE_MUSIC_TS_IMPORTANT_STOREFRONT_BOOST` (défaut x3) aux 11 storefronts affichés dans la fiche chanson (`us,gb,fr,ca,au,de,jp,br,mx,it,es`) pour que le rang global reste davantage ancré dans les marchés inspectables et moins dans la longue traîne des petits stores. |
| `ts_page_all.py` | **(runner, 2026-08-17 ; refonte collecte/publication 2026-09-19)** Même chart TS top songs mais agrégé sur **tous les storefronts découverts** (`core/storefronts.py::resolve_storefronts`, ~167 pays) en un classement composite (score `500/rank**0.75` par storefront, multiplié par poids marché puis sommé). **Agrégation par ISRC** (fallback `apple_music_id` si ISRC vide, depuis 2026-09-10) : Apple sert un `apple_music_id` différent par catalogue régional pour le même master (Wildest Dreams / Blank Space / Style… ont un id US, un international, un Japon-deluxe) ; keyer sur l'id éclatait la chanson en 2-3 entrées concurrentes, chacune ne portant que ses storefronts et une fraction du score. L'ISRC identifie le master indépendamment de la sortie → fusionne les repackagings deluxe/platinum/EP, garde les Taylor's Versions / remixes / lives séparés (ISRC propre). L'`apple_music_id` écrit au CSV = celui du fragment le mieux classé. Pagination plafonnée à `APPLE_MUSIC_TS_GLOBAL_DEPTH` (défaut **400** = 4 pages/storefront depuis 2026-09-10, avant 200 ; le catalogue TS complet fait ~675 titres/storefront, mais au-delà du rang ~400 le score composite se joue à des fractions de point = bruit). Le classement global est l'**union** des tops de tous les storefronts : une chanson est classée dès qu'elle apparaît dans les N premiers d'au moins un storefront. `--date`, `--scraped-at`, `--dry-run` (calcule et affiche le top 20, n'écrit rien) (plus de `--force`, plus de gate horaire — collecte à chaque cycle). **Fusion chart (2026-09-25)** : par storefront, les titres TS présents dans le chart Apple Music Top Songs du pays (top `CHART_LIMIT`=200, même endpoint que `country_all.py`, récupéré dans le même cycle) passent **en tête, dans l'ordre du chart**, puis le reste de la liste page-artiste dans son ordre (dédup par `apple_music_id` ou ISRC) — `merge_chart_into_top_songs`. Raison : la vue `top-songs` de la page artiste Apple met des heures à intégrer une nouvelle sortie (Showgirl Encore : #1-#4 du chart UK, absents de la page artiste UK jusqu'à l'après-midi → #105-#282 au TS Top Songs). TS Top Songs est notre chart, on peut mélanger. `ts_page.py` (input TayBoard) n'est PAS concerné. Chart en 400 = pas de chart dans ce pays → liste page-artiste seule. Depuis 2026-09-19, écrit un fichier **brut** `apple_music_ts_top_songs_global_raw.csv` à chaque cycle (jamais lu par l'export) ; c'est `finalize_ts_top_songs_daily.py` qui agrège ces cycles en `apple_music_ts_top_songs_global.csv` (le fichier lu par `export_apple_music.py`/`upload_ap_r2.py`), une fois par jour, une fois le jour précédent terminé. |
| `finalize_ts_top_songs_daily.py` | **(runner, 2026-09-19)** Agrège les cycles bruts (`*_raw.csv`) de **la veille** en une ligne finale par chanson dans le CSV canonique — score jour = somme de `500/rank**0.75` sur tous les cycles où la chanson est apparue ce jour-là (logique partagée dans `core/ts_top_songs_daily.py`). Idempotent (no-op si le jour a déjà des lignes canoniques, sauf `--force`), donc sans risque d'être appelé à chaque cycle : ne travaille réellement qu'une fois, au 1er cycle du lendemain. `--date`, `--scraped-at`, `--target-date` (override manuel du jour à finaliser), `--force`. Tant qu'un jour n'est pas finalisé, il n'a **aucune** ligne dans le CSV canonique — le site continue d'afficher le dernier jour complet. |
| `ts_top_songs_live.py` | **(runner, 2026-09-24)** Classement TS Top Songs **du jour en cours** = somme des cycles bruts déjà collectés aujourd'hui, via le **même** `compute_final_rows()` que la finalisation (après le dernier cycle du jour, il est identique au final — vérifié sur le 2026-09-23 : 11 cycles, 633 lignes identiques). Réécrit intégralement `apple_music_ts_top_songs_global_live.csv` (dossier du jour) avec 2 colonnes en plus : `live_cycles`, `compared_to_day` (`previous_rank` = vs dernier jour FINAL, comme le final). Lancé après `finalize_ts_top_songs_daily.py` (même groupe), non bloquant. Lu seulement par `export_apple_music.py` → clé séparée `ts_top_songs_live` de `applemusic.json` (`date`, `scraped_at`, `cycles`, `compared_to_day`, `final_date` = vraie clé du dernier final prise dans `ts_top_songs_dates` avant le mirroring, `entries`) ; jamais mélangé à `ts_top_songs`/historique, jamais lu par TayBoard/posting. Frontend : route `/api/apple-music/ts-top-songs-live` + toggle « Today (live) / Final · date » de l'onglet TS Top Songs (live par défaut à partir de 3 cycles). `--date`, `--scraped-at` (ignoré). Sim : `previews_and_sims/ts-top-songs-live/` (`simulate.py` = test d'identité + progression cycle par cycle ; `preview_api.py` = API locale réelle sur :8003 avec seul le payload live injecté) |
| `backfill_ts_top_songs_daily_final.py` | **(one-off, 2026-09-19)** A appliqué rétroactivement la même agrégation à tout l'historique (2026-07-30 → 2026-09-18, où chaque jour avait 4 à 45 cycles concurrents faute de la séparation brut/final ci-dessus). Réécrit chaque fichier jour en place (`.bak` gardé). Ne pas rejouer sauf bug trouvé dans l'agrégation. `--start`, `--end`, `--dry-run`. |
...
| `generate_snapshot_images.py` | PNG des snapshots. Colonne Peak sur toutes les cards depuis le 2026-09-25 (NEW PEAK / RE-PEAK / BEST IN 2026 ; jours passés cachés dans `tools/cache/peak_rows/`). Colonne Peak du Global : complétée par `db/apple_music_global_alltime_peaks.json` (peaks Global de tous les temps fournis par le proprio, titres sortis avant notre historique du 2026-03-22). `--date`, `--region us\|fr\|global…`, `--genre` (avec `--region`), `--album <nom\|slug\|sous-chaîne>` (ex. `showgirl` : seulement les titres de l'album, rangs d'origine, PNG suffixé, + colonne PEAK depuis la sortie avec badge NEW / NEW PEAK, « best in 2026 » sans badge si sorti avant l'historique AM, lu aussi dans le legacy `data/<jour>/apple_music/` à partir du 2026-03-22), `--list-genres`, `--out-dir`. Sous-titre = heure du snapshot + « vs » heure du dernier snapshot de la veille (Paris) |
| `post_new_release_progression.py` | **(2026-09-24)** lancé **une fois par plateforme** juste après son collecteur (côté Apple Music : **démarré par `run_apple_music.py` dès la fin des collecteurs**, en parallèle de l'export/upload/images, depuis le 2026-09-25 — plus une ligne du `.bat` ; `--platform itunes` en fin de `collectors/itunes/run_itunes.bat`, `--platform apple_music` en fin de `run_apple_music.bat` ; les 2 chaînes tournent en parallèle, état séparé `tools/json/new_release_progression_state_<platform>.json`, card album Global seulement côté Apple Music), seul script du pipeline Apple Music qui poste sur X. Pour tout titre catalogue dont `release_date` est dans les dernières `APPLE_MUSIC_DEBUT_WINDOW_HOURS` (déf. 72h), compare son rang Apple Music Global/pays clés + iTunes pays clés au dernier rang vu (état propre, pas le `previous_rank` du CSV) et poste une card si le rang a bougé (ou 1re apparition = DEBUT) — **une card/un post séparé par plateforme** (Apple Music vs iTunes), cover/titre d'album via `covers.json` + `display_title_for_album`. Card = portage de la share image du site (`.overall-song-block` AppleMusic.jsx/ITunes.jsx), footer `@swiftiescharts`, logo iTunes dans `collectors/apple_music/itunes_logo.svg`. Poste aussi, par album concerné, la card Global filtrée album (`generate_snapshot_images.generate(..., album=...)`) quand le classement de l'album sur le Global a changé depuis le dernier post. No-op hors fenêtre de sortie. `--platform {all,apple_music,itunes}` (déf. `all` pour un run manuel), `--scraped-at`, `--no-post` (rend les cards, n'écrit rien), `--dry-run` (texte seul, n'écrit rien), `--window-hours`. Depuis la nuit du 2026-09-24 : fenêtre démarrée à la sortie **détectée** (lookup iTunes `isStreamable`/`trackPrice` ou 1re apparition en chart), matching id Apple ou titre, état mis à jour seulement après un post réussi (retry), limitation de volume (`post_due` : debut/#1 immédiats, autres peaks regroupés `APPLE_MUSIC_DEBUT_PEAK_GAP_MINUTES` (15), montées `APPLE_MUSIC_DEBUT_MIN_GAP_HOURS`, `APPLE_MUSIC_DEBUT_POST_DROPS`), alertes ntfy. Depuis le 2026-09-25 : card album iTunes Top Albums (chaîne iTunes) et card Global normale Apple Music à chaque changement du Global pendant la fenêtre (chaîne Apple Music ; au 1er passage, comparée au cycle Global précédent du CSV) ; une sortie détectée par une chaîne vaut pour l'autre (état partagé en lecture) ; card album iTunes par pays (`_post_itunes_album_card`, déf. US : toutes les chansons de l'album dans le Top Songs du pays, au plus 1×/h via `ITUNES_ALBUM_CARD_GAP_MINUTES`, `ITUNES_ALBUM_CARD_REGIONS`) ; card Apple Music **Pop** par marché clé (`_post_pop_snapshot`, `APPLE_MUSIC_DEBUT_POP_COUNTRIES`, seulement #1 / entrée ou nouveau peak dans le top 10) ; card album Apple Music (Top Albums pays) ; chansons d'un même cycle postées en un thread par plateforme (`post_image_thread`, qui accepte maintenant `slot_timeout`). Simulation : `previews_and_sims/apple-music-debut-progression/release_night.py`. Détail → skill `collector-apple-music`. |
| `core/` | `token.py` (jeton API + `TokenManager` refresh coordonné pour les pools), `http.py`, `storefronts.py`, `filters.py`, `csv_utils.py` (`previous_rank` = dernier snapshot du **jour précédent**, fenêtre lecture 30 j), `discography.py` (2026-09-24, catalogue `db/discography/` + fenêtre release récente, partagé par `generate_snapshot_images.py` et `post_new_release_progression.py`), `card_theme.py` (2026-09-24, `THEMES` par album + `country_label`/`logo_data_uri`, extrait de `generate_country_card_images.py` pour que `post_new_release_progression.py` matche le même look — le générateur d'origine garde sa propre copie inline, non touché), `r2.py`, `config.py`, `export.py`, `models.py` |
...
(tâche `TSM Apple Music Every 4 Hours`, repeat 1h depuis le 2026-09-24) démarre **en parallèle, en premier**
`collectors/itunes/run_itunes.bat` (depuis le 2026-09-24 au soir ; avant : séquentiel après Apple Music) :
`run_itunes.py` puis `post_new_release_progression.py --platform itunes` — la chaîne iTunes n'attend plus
Apple Music (posts ~HH:03). Log `collectors/itunes/run_itunes.log`, indépendant du code de sortie
d'Apple Music. `run_itunes.bat` / `run_itunes_hidden.vbs` restent utilisables seuls (rattrapage manuel). Depuis le 2026-09-25 : verrou mono-instance (`collectors/itunes/tools/locks/run_itunes.lock`, sortie 75 = heure sautée, le `.bat` saute alors le post) et alertes ntfy sur tout échec.

| Fichier | Rôle / lancement |
|---|---|
| `release_watch.py` | **(2026-09-25)** attente de sortie, **appelée en fin de chaque `run_itunes.py`** (`run_if_release_due()`, après libération du verrou de collecte ; aucun process ni tâche séparés) : simple comparaison de date, sauf si un titre candidat du catalogue sort dans les ~70 min (sortie = 00:00 New York à la date catalogue, fenêtre −10 min → +2 h, ex. 05:50-08:00 Paris → c'est le run de 05:00 qui attend). Dans la fenêtre : 1 feed Top Songs d'un pays clé par seconde (US une seconde sur deux) + lookup toutes les 15 s ; dès qu'un titre apparaît → post **express** (Top Songs de **tous** les storefronts ~168, ~55 s à 3 requêtes parallèles, `collectors/apple_music/tools/json/express/`, `run_platform(express=...)`, card toutes régions + ligne mondiale ; la liste des storefronts est résolue avant la sortie ; le début de l'album vient du run complet juste après) + collecte complète `run_itunes.py` en parallèle (réessai tant que le verrou horaire répond 75) puis post normal. **Phase follow (2026-09-25, iTunes bouge en quelques minutes)** : après la sortie, et pendant `ITUNES_FOLLOW_HOURS` (24 h) — chaque run horaire (`--auto`) relance `follow()` jusqu'à HH:58:30 : 1 feed pays clé / `ITUNES_FOLLOW_INTERVAL` (2 s) ; rang confirmé par 2 lectures consécutives (le CDN Apple sert des versions différentes du feed), + 1 Top Albums pays clé sur 4 ticks ; post horaire normal lancé avant le follow ; dès qu'un pays clé montre un move que `post_due` posterait maintenant (debut / #1 immédiats, autre peak sauf post < 15 min, montée 3 h) vs le **dernier état posté** → re-fetch express des ~168 storefronts + `run_platform(express=...)` (baisses jamais déclenchées ; même mouvement toujours là après 120 s = post raté → réessai). Verrou mono-instance `tools/locks/release_watch.lock`. Contrôle pré-sortie à l'ouverture de la fenêtre (`preflight` : collectes fraîches, feeds/API joignables, disque, profil X) → une notif ntfy « armed » ou la liste des problèmes. 403/429 → pause 15→120 s. Mode manuel/test : `--until HH:MM`, `--interval`, `--dry-run`, `--watch-ids`. |
| `core/rss.py` (garde-fou 2026-09-25) | un feed US/UK de moins de `ITUNES_MIN_CRITICAL_FEED_ENTRIES` (déf. 50) entrées = échec (réessai puis abandon + alerte). Chaque étape (collecte, export, upload) est réessayée 3× (`run_guard.retry_step`) avant alerte ; posts X 3× (sauf « non confirmé » : jamais repost). Codes de sortie des runners : 0 ok, 3 échec déjà alerté, 75 heure sautée ; tout autre code → le `.bat` alerte lui-même via curl/ntfy. |
| `charts.py` | **(runner)** Top Songs + Top Albums par storefront (2 flux RSS), threadé (`ITUNES_WORKERS` déf. **3** — le RSS legacy throttle en **403** sous rafale). `--countries`, `--date`, `--scraped-at`. Un 404 = chart vide (pas un échec) ; un 403/429/503 = throttle → retries avec backoff dans `_fetch`, puis **passe de rattrapage séquentielle** des storefronts encore en échec (session neuve, 5 s d'espacement). Abort si : un storefront **critique** (`ITUNES_CRITICAL_STOREFRONTS` déf. `us,gb`) échoue après retry, OU taux d'échec > `ITUNES_MAX_FAILURE_PCT` (déf. **25%**). Sinon publie avec ce qu'on a (le snapshot suivant, 2 h après, backfille — `previous_rank` tolère les trous). Écrit `db/itunes_top_songs.csv` / `db/itunes_top_albums.csv` (idempotents par `scraped_at`, chemin réel = `snapshots/itunes_charts/AAAA/MM/AAAA-MM-JJ/`). **(2026-09-24)** Un même titre peut charter deux fois dans le même pays sous le même `album_name` mais avec un `apple_music_id` différent — Apple vend l'album en édition "explicit" et "cleaned" séparément, chacune sa propre fiche catalogue ; la RSS ne porte aucun flag explicit. `charts.py` détecte ces doublons (même pays + titre + album, id différent) et résout `collectionExplicitness` en un seul appel groupé à `core/lookup.py::resolve_explicitness` (iTunes Lookup API), écrit dans la colonne `explicitness` (`explicit`/`clean`/vide) du CSV songs — jamais résolu pour les entrées non ambiguës (pas d'appel superflu). Le frontend (`ITunes.jsx`) s'en sert pour séparer ces deux entrées en deux "éditions" distinctes au lieu de les fusionner à tort, avec un badge "E" sur l'édition explicite. |
...
| `core/rss.py` | Fetch + parse des flux RSS legacy (**`feed_url()` ajoute `?cb=<time_ns>` à chaque requête depuis 2026-09-25 : l'URL nue sert le cache Akamai, périmé ≥ 1 h** ; `im:id` → `apple_music_id`, artist id extrait de l'href `im:artist`, `im:releaseDate` gardé en ISO uniquement, artwork upscalé 300/500). `is_taylor_entry()`. `ITunesFeedError` = erreur réseau/5xx (comptée), pas un 404. |
...
| `core/lookup.py` | **(2026-09-24)** `resolve_explicitness(session, ids, country="us")` : un seul appel groupé (chunks de 150) à `itunes.apple.com/lookup?id=...` pour récupérer `collectionExplicitness` des `apple_music_id` ambigus (doublon même titre/album, id différent) détectés par `charts.py`. Jamais appelé pour les entrées non dupliquées. |
...
| `swift_top_100.py` (72 Ko) | **Chart hebdo Swift Top 100** (style Billboard). `--date` (fin de semaine), `--backfill` (+`--force`), `--streams-csv`, `--rebuild-index`, `--generate-songs`, `--dry-run`, `--skip-r2`, `--skip-images`, `--variant combined\|not-combined\|full-tayboard`. Expose aussi `compute_track_units()` (formule units_spotify/units_am/units_youtube/units_charts/units_surplus/total_units, extraite de `run()` le 2026-09-22) et `return_daily=True` sur les 6 fonctions de scoring hebdo (AM global/country/genre/ts, YouTube, charts streams) — réutilisés par `swift_top_100_live.py` |
| `swift_top_100_live.py` | **Projection live** (non officielle) de la semaine Fri->Thu en cours à partir des jours déjà réels + projection des jours restants (`live_projection.py`). `--date` (as-of, défaut aujourd'hui), `--dry-run`, `--skip-r2`. Variant combined uniquement dans son propre produit public (not-combined retiré 2026-09-23) — écrit `db/swift_top_100_live_history.csv` + `runtime/exports/web/site/data/swift_top_100_live.json` (+ copie datée), jamais les artefacts officiels. Expose aussi `_build_live_variant(..., include_album_agg=False)` : appelable avec `variant="not-combined"` et `include_album_agg=True` pour obtenir en mémoire (jamais écrit) le pool complet (non capé à 100) de chansons not-combined projetées, consommé par `swift_top_albums_live.py`. Pas de commit git |
| `live_projection.py` | Helpers stdlib de saisonnalité jour-de-semaine + momentum EWMA (`weekday_seasonal_factor`, `trend_momentum_factor`, `project_remaining_days`) utilisés par `swift_top_100_live.py` |
| `swift_top_albums_live.py` (ajouté 2026-09-23) | **Projection live Albums/Eras** (suite du songs-only, voir skill `collector-billboard` § "Albums/Eras live"). `--date`, `--dry-run`, `--skip-r2`. Réutilise `swift_top_100_live._build_live_variant(variant="not-combined", include_album_agg=True)` puis `swift_top_albums._build_album_week()` (la fonction du chart officiel) pour agréger en albums/eras. Écrit `db/swift_top_albums_live_history.csv` / `db/swift_top_eras_live_history.csv` + `runtime/exports/web/site/data/swift_top_albums_live.json` / `swift_top_eras_live.json` (+ copies datées), jamais les artefacts officiels. Pas de commit git, pas d'image PNG |
| `live_trigger.py` (ajouté 2026-09-23) | `trigger_live_projection(*, log=print)` : lance `swift_top_100_live.py` **et** `swift_top_albums_live.py` (ajouté à ce trigger le même jour) en sous-processus (run réel, pas de `--dry-run`/`--skip-r2`), chacun avec son propre best-effort/non-bloquant (try/except, jamais de raise, un échec ne bloque pas l'autre). Appelé en fin de run réussi par `run_apple_music.py`, `collectors/spotify/streams/update_streams.py` et `collectors/youtube/update_youtube.py` — remplace l'ancien schéma "un run/jour sur horaire fixe" par un déclenchement après chaque collecteur individuel |
...
| `upload_ap_r2.py` | Upload Apple Music vers R2 (JSON, snapshots par date, CSV du jour, history-by-song incl. vidéos). `--bucket`, `--prefix`, `--dry-run`. Concurrence par phase `APPLE_MUSIC_R2_WORKERS` (défaut 24) ; le build per-song tourne en tâche de fond pendant les phases précédentes (ordre d'upload inchangé). Gzip R2 (`ContentEncoding: gzip`, hash `sha256` sur le JSON non compressé) : per-song, `applemusic_history.json` et, depuis 2026-09-24, `apple-music/history-by-date/*.json` (hors `index.json`) ; un objet de même contenu pas encore gzippé est ré-uploadé une fois (auto-migration). `r2.py` gzippe le même préfixe (`_GZIP_KEY_PREFIXES`) pour ne pas refaire basculer le format |
...
**État actuel (2026-09-24) : `collectors/youtube` et `collectors/apple_music`
...
**1h depuis le 2026-09-24** (avant 2h) — **lance aussi
la chaîne iTunes (`collectors/itunes/run_itunes.bat`) EN PARALLÈLE depuis le 2026-09-24 au soir** (avant : juste après, depuis le 2026-09-09), les deux
appels python en `-u` depuis le 2026-09-24 pour éviter un log bufferisé qui
parait figé pendant tout le run ; `TSM YouTube Videos Daily` : `run_youtube.bat`,
06:05 Europe/Paris). Le passage à la cadence horaire nécessite aussi
`APPLE_MUSIC_SNAPSHOT_HOURS` / `ITUNES_SNAPSHOT_HOURS=0,1,...,23` dans `.env`
(le code par défaut arrondit `scraped_at` aux heures paires uniquement —
détail dans skills `collector-apple-music` / `collector-itunes`).
