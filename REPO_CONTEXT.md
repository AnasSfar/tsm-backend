...
| `ts_page_all.py` | **(runner, 2026-08-17)** Même chart TS top songs mais agrégé sur **tous les storefronts découverts** (`core/storefronts.py::resolve_storefronts`, ~167 pays) en un classement composite (score `500/rank**0.75` par storefront, multiplié par poids marché puis sommé). Écrit `apple_music_ts_top_songs_global.csv`, séparé exprès du fichier single-storefront ci-dessus pour ne pas polluer le scoring TayBoard. **Agrégation par ISRC** (fallback `apple_music_id` si ISRC vide, depuis 2026-09-10) : Apple sert un `apple_music_id` différent par catalogue régional pour le même master (Wildest Dreams / Blank Space / Style… ont un id US, un international, un Japon-deluxe) ; keyer sur l'id éclatait la chanson en 2-3 entrées concurrentes, chacune ne portant que ses storefronts et une fraction du score. L'ISRC identifie le master indépendamment de la sortie → fusionne les repackagings deluxe/platinum/EP, garde les Taylor's Versions / remixes / lives séparés (ISRC propre). L'`apple_music_id` écrit au CSV = celui du fragment le mieux classé. Pagination plafonnée à `APPLE_MUSIC_TS_GLOBAL_DEPTH` (défaut **400** = 4 pages/storefront depuis 2026-09-10, avant 200 ; le catalogue TS complet fait ~675 titres/storefront, mais au-delà du rang ~400 le score composite se joue à des fractions de point = bruit). Le classement global est l'**union** des tops de tous les storefronts : une chanson est classée dès qu'elle apparaît dans les N premiers d'au moins un storefront. `ts_page_all.py` garde un gate interne une fois/jour (`APPLE_MUSIC_TS_GLOBAL_HOUR`, défaut `02`), mais `run_apple_music.py` lui passe `--force` à chaque run depuis le passage GitHub Actions data-only afin que l'export R2 ait toujours un composite frais. `--date`, `--scraped-at`. Le fichier brut conserve désormais `composite_score` à chaque snapshot de 2 h. `daily_top_songs.py` exige tous les créneaux de `APPLE_MUSIC_SNAPSHOT_HOURS`, moyenne ces scores exacts et écrit `apple_music_ts_top_songs_daily.csv` à partir de 10 h Europe/Paris (`APPLE_MUSIC_TS_DAILY_PUBLISH_HOUR`) pour la veille ; ce fichier quotidien est celui que `export_apple_music.py`/`upload_ap_r2.py` lisent pour l'onglet site "TS Top Songs". Si un créneau/score manque, la finalisation est bloquée et la dernière édition valide reste publiée |
...
| `core/` | `token.py` (jeton API + `TokenManager` refresh coordonné pour les pools), `http.py`, `storefronts.py`, `filters.py`, `csv_utils.py` (`previous_rank` brut = dernier snapshot du **jour précédent**, fenêtre lecture 30 j ; option `skip_identical=False` pour conserver les observations du chart quotidien), `r2.py`, `config.py`, `export.py`, `models.py` |
