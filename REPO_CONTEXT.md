...
(tâche `TSM Apple Music Every 4 Hours`, repeat 1h depuis le 2026-09-24) lance `run_itunes.py`
...
| `upload_ap_r2.py` | Upload Apple Music vers R2 (JSON, snapshots par date, CSV du jour, history-by-song incl. vidéos). `--bucket`, `--prefix`, `--dry-run`. Concurrence par phase `APPLE_MUSIC_R2_WORKERS` (défaut 24) ; le build per-song tourne en tâche de fond pendant les phases précédentes (ordre d'upload inchangé) |
...
`collectors/itunes/run_itunes.py` juste après depuis le 2026-09-09**, les deux
