...
| `finalize_update.py` | Étapes de finalisation post-collecte (exports, home highlights, forecast) + **orchestration des posts**. Ordre (`run_final_update_tasks`, décision 03/09/2026) : 1. récap weekend (`_post_daily_recap_card`) 2. top eras 3. top songs (top 20) 4. **alternance 1-pour-1** entre la file album `_album_post_queue` (2 meilleurs au score → Showgirl/TTPD → reste par score, hors week-end) et les autres posts (debut, best-day-since, weekend gainers, overtakes, milestones) 5. tables gainers spotlight, toujours en dernier. `POST_ONLY_STEPS` = étapes rejouables via `--post-only`. |
