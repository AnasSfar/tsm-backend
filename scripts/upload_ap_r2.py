...
def build_history_objects() -> dict[str, dict[str, Any]]:
    ...
    append_rows(
        grouped=grouped,
        rows=read_csv(COUNTRY_ALBUMS_CSV),
        source_name="country_albums",
        keep_fields=["date", "scraped_at", "country", "album_name", "rank", "previous_rank", "image_url", "url", "apple_music_id"],
    )
    ...
    append_rows(
        grouped=grouped,
        rows=read_csv(GLOBAL_ALBUMS_CSV),
        source_name="global_albums",
        keep_fields=["date", "scraped_at", "country", "album_name", "rank", "previous_rank", "image_url", "url", "apple_music_id"],
    )

    append_rows(
        grouped=grouped,
        rows=read_csv(MUSIC_VIDEO_CHARTS_CSV),
    ...
    append_rows(
        grouped=grouped,
        rows=read_csv(TS_TOP_VIDEOS_CSV),
        source_name="ts_top_videos",
        keep_fields=["date", "scraped_at", "storefront", "video_name", "rank", "previous_rank", "image_url", "url", "apple_music_id"],
    )
